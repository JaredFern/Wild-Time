#!/usr/bin/bash
#SBATCH --job-name=mimic-mortality
#SBATCH --output=logs/mimic-mortality.out
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# ERM Reproduction
python main.py \
    --train --eval_fix --exp_name erm_baseline \
    --dataset $DATASET --method erm --random_seed $SEED \
    --offline_steps $OFFLINE_STEPS;

# ERM + Finetuning
python main.py \
    --train --eval_warmstart_finetune --exp_name erm_ft \
    --dataset $DATASET --method erm --random_seed $SEED \
    --offline_steps $WARMSTART_STEPS --online_steps $ONLINE_STEPS;

# Finetuning
python main.py \
    --train --eval_warmstart_finetune --exp_name ft\
    --dataset $DATASET --method erm --random_seed $SEED \
...
python main.py \
    ...
    --offline_steps $OFFLINE_STEPS --online_steps $OFFLINE_STEPS;

# ERM + SWA FT
python main.py \
    --train --eval_warmstart_finetune --exp_name erm_ordered_swa \
    --dataset $DATASET --method swa --swa_steps $OFFLINE_SWA_STEPS --random_seed $SEED \
    --offline_steps $WARMSTART_STEPS --online_steps $OFFLINE_STEPS;

# EWA
python main.py \
    --train --eval_fix --exp_name unordered_ewa \
...
# EWA (Shuffle Buckets)
python main.py \
    --train --eval_warmstart_finetune --shuffle_timesteps  --exp_name erm_shuffled_ewa \
...
# Cold Start and EWA
python main.py \
    --train --eval_warmstart_finetune --exp_name coldstart_csaw \
    --dataset $DATASET --random_seed $SEED \
    --method swa --swa_ewa --swa_ewa_lambda 0.50 \
...
# Lamba 0.1
python main.py \
    --train --eval_warmstart_finetune  --exp_name csaw_lambda0.1 \
    --dataset $DATASET --random_seed $SEED \
...
# Lamba 0.25
python main.py \
    --train --eval_warmstart_finetune --exp_name csaw_lambda0.25 \
    --dataset $DATASET --random_seed $SEED \
...
# Lambda 0.75
python main.py \
    --train --eval_warmstart_finetune --exp_name csaw_lambda0.75 \
    --dataset $DATASET --random_seed $SEED \
...
# Lamba 0.90
python main.py \
    --train --eval_warmstart_finetune --exp_name csaw_lambda0.90 \
    --dataset $DATASET --random_seed $SEED \
...
# SAM Optimizer
python main.py \
    --train --eval_warmstart_finetune --exp_name csaw_sam \
    --dataset $DATASET --random_seed $SEED \
...
python main.py \
    --train --eval_warmstart_finetune --exp_name csaw_last0.1 \
    --dataset $DATASET --random_seed $SEED \
...
python main.py \
    --train --eval_warmstart_finetune --exp_name csaw_last0.25 \
    --dataset $DATASET --random_seed $SEED \
